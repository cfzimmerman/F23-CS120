...
# Encodes a node as a color


def encolor(node: int, color: int) -> int:
    return node * 10 + color


# Decodes a (node, color) pair
def decolor(encolored: int) -> Tuple[int, int]:
    return (encolored // 10, encolored % 10)
...
def sat_3_coloring(G):
    ...
    for node in range(G.N):
        clause = [encolor(node, color) for color in three_colors]
        solver.add_clause(clause)

    # the endpoints of an edge cannot have the same color
    for node, edges in enumerate(G.edges):
        for neighbor in edges:
            for color in three_colors:
                clause = [
                    -encolor(node, color),
                    -encolor(neighbor, color),
                ]
                solver.add_clause(clause)
